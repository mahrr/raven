...
struct Parser {
    ...
    token *prev;
    token *peek;
    List_T tokens;  /* of token */
    List_T errors;  /* of Parse_error */
...
/* fetch the current token and increment the token 
   list ignoring any newlines tokens */
...
static token *match_token(Parser p, token_type t) {
...
static int curr_token_not(Parser p, int n, va_list ap) {
    for (int i = 0; i < n; i++) {
        token_type t = va_arg(ap, token_type);
...
/* return expected token if it has type 't' then increment 
   ...
   message if not. */
static token *expect_token(Parser p, token_type t, char *expected) {
    if (curr_token_is(p, t)) {
...
static List_T patterns(Parser, token_type, token_type, char*);
...
static AST_patt hash_patt(Parser p) {
    ...
    List_T names = List_new(R_SECN);
    List_T patts = List_new(R_SECN);
...
static AST_patt list_patt(Parser p) {
    ...
    List_T patts = patterns(p, TK_COMMA, TK_RBRACKET, "]");
...
static List_T expressions(Parser, token_type, token_type, char*);
...
static AST_expr call_expr(Parser p, AST_expr func) {
    ...
    List_T args = expressions(p, TK_COMMA, TK_RPAREN, "')'");
...
static AST_expr match_expr(Parser p) {
    ...
    List_T branches = List_new(R_SECN);
...
static AST_expr fn_literal(Parser p) {
    ...
    List_T params = patterns(p, TK_COMMA, TK_RPAREN, ")");
...
static AST_expr list_literal(Parser p) {
    ...
    List_T values = expressions(p, TK_COMMA, TK_RBRACKET, "]");
...
static List_T
patterns(Parser p, token_type dl, token_type end, char *end_name) {
    List_T patts = List_new(R_SECN);
    AST_patt patt;
...
/* return list of zero or more AST_expr delimited by 
   'dl' token type and ended witn 'end' token type */
static List_T
expressions(Parser p, token_type dl, token_type end, char *end_name) {
    List_T exprs = List_new(R_SECN);
    AST_expr expr;
...
Parser parser_new(List_T tokens, Region_N reg) {
    Parser p = make(p, reg);
    p->tokens = tokens;
